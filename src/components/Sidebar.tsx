...
export function Sidebar({
  ...
  return (
    <aside
      className={`${collapsed ? "w-16" : "w-56"} bg-sidebar flex flex-col h-full flex-shrink-0 transition-[width] duration-200`}
    >
      {/* Logo + 折叠按钮 */}
      <div
        className={`flex items-center border-b border-slate-700/50 ${
          collapsed ? "flex-col gap-2 px-2 py-3" : "px-5 py-4"
        }`}
      >
        ...
        <button
          ...
          className={`text-slate-400 hover:text-white hover:bg-white/10 rounded-md transition-colors ${
            collapsed ? "w-7 h-7 flex items-center justify-center" : "p-1"
          }`}
        >
        ...
        <div className="space-y-0.5">
          {navItems.map((item) => (
            ...
            >
              ...
              {!collapsed && item.key === "notifications" && notificationsCount > 0 && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
              ...
              {!collapsed && item.key === "settings" && updateAvailable && (
                <span className="text-[10px] min-w-[16px] h-4 px-1 flex items-center justify-center rounded-full bg-red-500 text-white font-semibold leading-none">
                  1
                </span>
              )}
            </button>
      ...
      {/* System Status：折叠时仅保留状态圆点（title 提示引擎名） */}
      <div className="px-4 py-3 border-t border-slate-700/50">
        <div className="flex items-center gap-2 text-xs">
          <div className={`w-2 h-2 rounded-full ${dotColor} ${isConnected ? "animate-pulse" : ""}`} />
          {!collapsed && <span className="text-slate-400">Docker Engine</span>}
          {!collapsed && (
