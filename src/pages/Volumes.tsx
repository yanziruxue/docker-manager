...
export function Volumes({ volumes, loading, error, engineId, defaultVisibleColumns, onRefresh }: VolumesProps) {
  ...
  /** 把 formatBytes 产物（如 "12.3 MB"）解析回字节数，用于「总占用空间」汇总 */
  const parseSizeToBytes = (s?: string): number | null => {
    if (!s || s === "—") return null;
  ...
  const sizedBytes = volumes.map((v) => parseSizeToBytes(v.size)).filter((n): n is number => n !== null);
  const totalSizeBytes = sizedBytes.reduce((a, b) => a + b, 0);
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-700">{sizedBytes.length > 0 ? formatBytes(totalSizeBytes) : "—"}</p>
              <p className="text-xs text-slate-400">总占用空间{sizedBytes.length < volumes.length && volumes.length > 0 ? `（${sizedBytes.length}/${volumes.length} 已统计）` : ""}</p>
            </div>
          </div>
        </div>
      </div>
                  ...
                  {visibleColumns.has("name") && (
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2 min-w-0">
                        <Database size={14} className={`shrink-0 ${volume.inUse ? "text-blue-400" : "text-slate-300"}`} />
                        <span
                          title={volume.name}
                          className="text-sm font-medium text-slate-700 font-mono truncate max-w-[220px]"
                        >
                          {volume.name}
                        </span>
