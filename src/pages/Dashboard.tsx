...
export function Dashboard({ containers, stacks, images, activities, resourceStats, onNavigate, loading, error }: DashboardProps) {
  ...
  // 资源监控：真实引擎数据（CPU/内存为运行容器合计，磁盘分镜像/数据卷占用，网络为实时速率）
  ...
  const resourceRows: ResourceRow[] = resourceStats
    ? [
        ...
        {
          label: "Docker 镜像占用",
          display: fmtMB(resourceStats.imageDiskMB),
          note: `${images.length} 个本地镜像`,
          icon: <HardDrive size={16} />,
          color: "amber",
        },
        {
          label: "Docker 数据卷占用",
          display: fmtMB(resourceStats.volumeDiskMB),
          note: "命名卷合计",
        ...
        {
          label: "网络 I/O",
          display: `↓ ${fmtKB(resourceStats.netRxKBps)}/s / ↑ ${fmtKB(resourceStats.netTxKBps)}/s`,
          note: "运行容器实时速率",
