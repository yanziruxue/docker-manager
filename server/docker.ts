...
interface PullTaskInternal extends PullTaskInfo {
  ...
  /** 输出行缓冲：layerId 相同的进度行原地覆盖（同层只保留最新一行） */
  outputLines: { layerId?: string; text: string }[];
}
...
/**
 * 追加一行输出（带缓冲上限）。
 * 带 layerId 的进度行原地覆盖同层旧行：docker CLI / compose 每次刷新都重打整行，
 * 不去重的话 outputTail 会被同一层的几百条重复行刷屏（大小原地变化才符合直觉）。
 */
function pushOutput(task: PullTaskInternal, line: string, layerId?: string): void {
  if (layerId) {
    for (let i = task.outputLines.length - 1; i >= 0; i--) {
      if (task.outputLines[i].layerId === layerId) {
        task.outputLines[i].text = line;
        task.outputTail = task.outputLines.slice(-60).map((l) => l.text);
        return;
      }
    }
  }
  task.outputLines.push({ layerId, text: line });
...
function handleCliPullLine(task: PullTaskInternal, line: string): void {
  // 先识别层行：层进度行走「同层原地覆盖」，非层行（Pulling from / Digest / Status）只追加
  const m = line.match(/^([0-9a-f]{7,64}): (.+)$/);
  pushOutput(task, line, m ? m[1] : undefined);
  if (!m) return;
  ...
  docker.pull(pullImage, (err: any, stream: any) => {
    ...
    docker.modem.followProgress(
      ...
      (evt: any) => {
        ...
        pushOutput(task, line, evt.id || undefined);
        if (evt.id) {
          // 按层 ID 聚合状态
...
/** 卷大小缓存：engineId -> { at, sizes }。`docker system df -v` 要对每个卷跑 du，量大时偏慢，
 *  60 秒内重复刷新（切页 / 多端打开）直接用缓存，避免每次都触发全量统计。 */
const volumeSizeCache = new Map<string, { at: number; sizes: Map<string, number> }>();
const VOLUME_SIZE_CACHE_TTL_MS = 60_000;

function getVolumeSizes(engine: DockerEngine): Map<string, number> {
  const cached = volumeSizeCache.get(engine.id);
  if (cached && Date.now() - cached.at < VOLUME_SIZE_CACHE_TTL_MS) return cached.sizes;
  const sizes = getVolumeSizesViaCli(engine) || new Map<string, number>();
  volumeSizeCache.set(engine.id, { at: Date.now(), sizes });
  return sizes;
}

/** 通过 `docker system df -v` 解析「Local Volumes space usage」表格，返回卷名 -> 字节数。
 *  与镜像拉取铁律一致：SSH 引擎跑远程 CLI，TCP 引擎本地 CLI + DOCKER_HOST，socket 引擎本地 CLI。
 *  失败（CLI 缺失 / 超时）不抛出，仅返回 null，前端大小列显示「—」。 */
function getVolumeSizesViaCli(engine: DockerEngine): Map<string, number> | null {
  let out: string;
  try {
    if (engine.connectionType === "ssh") {
      out = sshExec(engine, "docker system df -v", { timeout: 30000 });
    } else {
      ...
      out = run("docker", ["system", "df", "-v"], { env, timeout: 30000 });
    }
  } catch (e: any) {
    console.warn(`获取卷大小失败（大小列将显示为 —）：${e.message}`);
    return null;
  }
  ...
  for (const raw of out.split("\n")) {
    ...
    if (/^VOLUME NAME\s+LINKS\s+SIZE/.test(line)) { inVolumeSection = true; continue; }
    if (!inVolumeSection) continue;
    if (!line) { inVolumeSection = false; continue; } // 空行 = 表格段落结束
    // 卷名不含空格，行尾两个字段固定为 LINKS(整数) 和 SIZE(如 12.3MB / 0B)
    const m = line.match(/^(.+?)\s+(\d+)\s+([\d.]+)\s*([kKMGTP]?B)$/);
    if (m) sizes.set(m[1].trim(), parseDockerSizeToBytes(m[3], m[4]));
  }
...
/**
 * 堆栈操作（流式版本）：供 SSE 实时进度弹窗使用。
 *
 * 与 `stackAction` 的区别：
 * - 用异步 `spawn` 代替 `spawnSync`（不阻塞事件循环，SSE 才能边执行边推送）；
 * - stdout/stderr 每产出一块就通过 `onChunk` 实时回调（compose 进度几乎全在 stderr）；
 * - 支持一次执行多个动作（如 强制更新 = pull + up），每步输出前加 `$ 命令` 标头，
 *   与 `stackAction` 的多步格式保持一致；
 * - restart 同样拆为 down + up 两步执行。
 *
 * 拉取可见性：up / pull / build 步骤如果产生镜像层下载进度，会自动注册一个
 * 合成拉取任务（复用镜像拉取任务系统），用户点「后台运行」后到镜像管理页
 * 仍能看到实时拉取进度。
 *
 * 全部步骤成功时 resolve 完整合并输出；任一步失败时 reject，
 * Error.output 携带「已推送的全部输出」（与 stackAction 的失败语义一致）。
 */

/** compose 非 tty 进度行：`b1d7b6c24718 Downloading 1.049MB/299MB`（层 ID + 状态 + 进度） */
const COMPOSE_LAYER_LINE_RE = /(?:#?\d+\s+)?([0-9a-f]{7,64})\s+(Downloading|Extracting|Pull complete|Already exists|Download complete|Verifying|Waiting|Retrying)\b/;

/**
 * 为堆栈 compose up/pull 注册合成拉取任务（懒创建：首次出现层进度行时才建，
 * 避免每个普通 up 都在镜像页留任务条）。之后层进度经 handleCliPullLine 聚合，
 * 与镜像页原生拉取任务共用同一套展示/轮询/超时清理。
 */
function ensureStackPullTask(engineId: string, stackName: string, stepArgs: string[], cache: { task?: PullTaskInternal }): PullTaskInternal {
  if (cache.task) return cache.task;
  const id = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 6)}`;
  const task: PullTaskInternal = {
    id,
    engineId,
    image: `${stackName}（compose ${stepArgs.join(" ")}）`,
    status: "pulling",
    startedAt: Date.now(),
    layers: [],
    outputTail: [],
  ...
  pullTasks.set(id, task);
  pushOutput(task, `$ docker compose ${stepArgs.join(" ")}`);
  cache.task = task;
  return task;
}

export async function stackActionStream(
  engine: DockerEngine,
  stackName: string,
  ...
  const runStep = (stepArgs: string[], timeout: number, pullSink?: { cache: { task?: PullTaskInternal } }): Promise<string> =>
    new Promise((resolve, reject) => {
      ...
      child.on("error", (err) => {
        const text = `${header}\n${err.message}`;
        if (pullSink?.cache.task) {
          pullSink.cache.task.status = "error";
          pullSink.cache.task.endedAt = Date.now();
          pullSink.cache.task.error = err.message;
          pushOutput(pullSink.cache.task, `✗ ${err.message}`);
        }
        reject(Object.assign(new Error(text), { output: text }));
      });
      child.on("close", (code, signal) => {
        if (pullSink?.cache.task) {
          const t = pullSink.cache.task;
          t.endedAt = Date.now();
          if (code === 0) {
            t.status = "success";
            pushOutput(t, `✓ 完成: ${stackName}`);
          } else if (t.status === "pulling") {
            t.status = "error";
            t.error = signal ? `执行超时被终止（${signal}）` : `退出码 ${code}`;
            pushOutput(t, `✗ ${t.error}`);
          }
        }
        if (code === 0) {
          const out = merged.trim() || "(无输出)";
          chunks.push(`${header}\n${out}`);
          resolve(out);
        } else {
          // signal 非空 = 被 timeout 杀掉（compose 执行超时），提示比 code=null 更明确
          const reason = signal ? `执行超时被终止（${signal}）` : `退出码 ${code}`;
          const text = `${header}\n${merged.trim() || reason}`;
          reject(Object.assign(new Error(text), { output: text }));
        }
      });
  ...
  // 展开动作序列：restart = down + up；down 超时 60s，其余 300s（与 stackAction 一致）
  const expanded: string[][] = [];
  for (const a of actions) {
    if (a === "restart") {
      expanded.push(["down"], ["up", "-d"]);
    } else {
      expanded.push(getComposeArgs(a));
    }
  }
  // up / pull / build 共用一个合成任务缓存（同一次操作的多步汇聚到一个任务条）
  const pullSink = { cache: {} as { task?: PullTaskInternal } };
  for (const stepArgs of expanded) {
    const pulls = stepArgs.includes("pull") || stepArgs.includes("up") || stepArgs.includes("build");
    await runStep(stepArgs, stepArgs[0] === "down" ? 60000 : 300000, pulls ? pullSink : undefined);
  }
  return chunks.join("\n\n");
}
...
function pruneImagesViaCli(engine: DockerEngine, all = false): { ImagesDeleted: any[]; SpaceReclaimed: number } {
  ...
  for (const rawLine of out.split("\n")) {
    ...
    if (line.startsWith("deleted:")) {
      deleted.push({ Deleted: line.slice("deleted:".length).trim() });
    } else if (line.startsWith("Total reclaimed space:")) {
      const m = line.match(/([\d.]+)\s*(\w+)/);
      if (m) space = parseDockerSizeToBytes(m[1], m[2]);
    }
  }
...
function parseDockerSizeToBytes(val: string, unit: string): number {
 ...
 * - 网络 I/O 为实时速率：保留上次采样快照（按引擎），差分除以间隔秒数；
 *   首次采样无基线时为 0，容器重启计数器回卷导致差分为负时钳为 0
 */
...
export async function getEngineResourceStats(engine: DockerEngine): Promise<any> {
  ...
  // Docker 磁盘占用：镜像与数据卷分开统计（df 的 Containers.Size 已含在镜像里，避免重复计）
  let imageDiskBytes = 0;
  let volumeDiskBytes = 0;
  if (df) {
    for (const img of df.Images || []) imageDiskBytes += img.Size || 0;
    for (const vol of df.Volumes || []) volumeDiskBytes += vol.UsageData?.Size || 0;
  }
  // /system/df 拿不到卷大小时（部分版本 UsageData 为空），回退 CLI system df -v（带 60s 缓存）
  if (volumeDiskBytes === 0 && (df?.Volumes?.length ?? 0) > 0) {
    for (const bytes of getVolumeSizes(engine).values()) volumeDiskBytes += bytes;
  }

  const ncpu = info.NCPU || info.CPUs || 1;
  const memTotalMB = Math.round((info.MemTotal || 0) / 1024 / 1024);

  // 网络 I/O 实时速率：与上次采样快照差分（SSE 1s 一采，REST 请求同样受益）
  const now = Date.now();
  const prev = lastNetSnapshot.get(engine.id);
  let netRxKBps = 0;
  let netTxKBps = 0;
  if (prev && now > prev.ts) {
    const secs = (now - prev.ts) / 1000;
    netRxKBps = Math.max(0, (netRxKB - prev.rxKB) / secs);
    netTxKBps = Math.max(0, (netTxKB - prev.txKB) / secs);
  }
  ...
  return {
    ...
    memoryUsageMB,
    imageDiskMB: Math.round(imageDiskBytes / 1024 / 1024),
