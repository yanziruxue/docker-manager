 ...
 * 堆栈操作（SSE 流式实时进度）
 *
 * 前端点击启动/停止/重启/拉取/构建（含多步组合操作）时立即建立 EventSource，
 * 后端用异步 spawn 执行 compose，边执行边推送输出块，弹窗实时滚动显示。
 *
 * - query `steps`：逗号分隔的动作序列（如 `pull,up`），默认取单个动作
 * - 事件：`chunk` = 输出块（增量文本）；`done` = 全部成功（完整输出）；
 *   `fail` = 失败（含已产生输出的错误详情）
 * - 连接断开只停止推送，compose 进程继续执行完毕（不影响实际操作）
 */
app.get("/api/engines/:id/stacks/:name/actions/stream", async (req, res) => {
  const engine = getEngine(req.params.id);
  if (!engine) { res.status(404).json({ success: false, error: "引擎不存在" }); return; }
  const { name } = req.params;
  const validActions = ["up", "down", "pull", "restart", "build"];
  const stepsParam = String(req.query.steps || "");
  const actions = (stepsParam ? stepsParam.split(",") : []).map((s) => s.trim()).filter((s) => validActions.includes(s));
  if (actions.length === 0) {
    res.status(400).json({ success: false, error: `无效的操作序列: ${stepsParam || "(空)"}` });
    return;
  }
  ...
  // 客户端断开后停止推送（compose 继续执行，只是不再写响应）
  let closed = false;
  ...
  try {
    const output = await stackActionStream(engine, name, actions as any, (chunk) => send("chunk", chunk));
    send("done", output);
  } catch (err: any) {
    send("fail", err?.output || err?.message || "操作失败");
  } finally {
    if (!closed) res.end();
  }
});
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const cur = getUpdateState();
      if (cur.phase === "downloading" || cur.phase === "extracting" || cur.phase === "replacing") {
        res.status(409).json({ success: false, error: "更新正在进行中，请稍后再试" });
        return;
      }
      // 仅落盘为待应用包，不触发更新；zip 校验与魔数检查在 saveUploadPackage 内完成
      const info = saveUploadPackage(buf);
      // 安排自动销毁倒计时（2 分钟内未手动点击「更新」则服务端自动清除文件）
...
/** 查询已上传、待应用的本地更新包 */
app.get("/api/system/update/local", (_req, res) => {
  res.json({ success: true, data: getPendingUpload() });
});

/** 主动丢弃已上传、待应用的本地更新包（手动「丢弃」按钮或前端倒计时归零时调用） */
app.delete("/api/system/update/local", (_req, res) => {
  discardPendingUpload();
  res.json({ success: true, data: { message: "已丢弃待应用更新包" } });
});

/** 应用已上传的本地更新包（手动点击「更新」触发，跳过 GitHub 下载） */
app.post("/api/system/update/apply-local", (_req, res) => {
  // 已在更新中：直接返回，不重复触发
  const cur = getUpdateState();
  if (cur.phase === "downloading" || cur.phase === "extracting" || cur.phase === "replacing") {
    res.json({ success: true, data: { message: "更新正在进行中", inProgress: true } });
  ...
  if (!pending.exists) {
    res.status(400).json({ success: false, error: "没有待应用的更新包，请先上传本地更新包" });
    return;
  }
  // 取消自动销毁定时器，避免倒计时到点误删正在应用的包
  cancelPendingExpiry();
  // 后台执行、立即返回（与 /apply 一致，进度由前端轮询 /system/update/status 获取）
  performUpdateFromUpload(getPendingUploadPath()).catch((err: any) => {
    // 已知失败路径 applyLocalZip 内部已 setState("error")；此处仅兜底极端异常
    markUpdateError("本地更新包应用失败", err?.message || String(err));
  });
  res.json({ success: true, data: { message: "更新已开始" } });
});
...
app.post("/api/engines/:id/images/prune", async (req, res) => {
  ...
  const label = all ? "清理未使用镜像" : "清理悬空镜像";
  apiLog.info(`${label} | 引擎=${engine.name}`);
  try {
    const result = await pruneImages(engine, { all });
    apiLog.info(`${label}完成 | 引擎=${engine.name} 结果=${JSON.stringify(result).slice(0, 200)}`);
    res.json({ success: true, data: result });
  } catch (err: any) {
...
const server = app.listen(PORT, () => {
  ...
  // 启动时若存在上次遗留的待应用更新包，按剩余 TTL 安排自动销毁（重启后定时器已丢失，需重建）
  schedulePendingExpiry();
});
